...
class AudioRecorder:
    ...
    def record(self) -> None:
        ...
        if it is greater than threshold.
        """
        ...
        try:
            while self.open == True:
                ...
                if (
                    state_loudness["play_alarm"]
                    and self.audio_loudness_label is not None
                ):
                    self.audio_loudness_label.configure(
                        text=Path.TEXT_GUI_AUDIO_5.value + str(state_loudness["db"]),
                        text_color="red",
                    )
                else:
                    self.audio_loudness_label.configure(
