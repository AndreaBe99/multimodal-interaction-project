...
fig, axs = plt.subplots(
    nrows=len(ACTIVITY.items()) // 5, ncols=len(ACTIVITY.items()) // 2, figsize=(20, 8)
)
...
df["file_path"] = df.apply(
    lambda x: os.path.join(data_dir, "imgs/train", x.classname, x.img), axis=1
)
...
df_train, df_val = train_test_split(
    df, train_size=0.8, test_size=0.2, stratify=df["subject"], random_state=SEED
)
